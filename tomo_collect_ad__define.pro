pro tomo_collect_ad::start_scan
    ...
    if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
        ...
        while (busy ne 0) do begin
            busy = self.scan.ccd->getProperty('Acquire_RBV', string = 0)
            if (busy ne 0) then begin ; attempt to take the image again if the initial attempt failed
                acq_time = self.scan.ccd->getProperty('AcquireTime', string = 0)
                wait, .1 + acq_time
                busy = self.scan.ccd->getProperty('Acquire_RBV', string = 0)
                if (busy ne 0) then begin
                   self.scan.ccd->setProperty,'Acquire',1
                   wait, .1
                endif
            endif
...
;        self.widgets.dc_timer = self.widgets.flatfield_base
...
pro tomo_collect_ad::dark_current
...
pro tomo_collect_ad::PrepareScan
    ...
    ; set number of flat fields in each flat field scan <=180, >1 for OTF scans, and >0 for Fast/slow scans
    if(self.scan.num_flatfields gt 180) then begin
        self.scan.num_flatfields = 180
        widget_control, self.widgets.num_flatfields, set_value=self.scan.num_flatfields
    endif
    if(self.scan.num_flatfields eq 1 AND self.scan.fast_scan eq 2) then begin
        self.scan.num_flatfields = 2
        widget_control, self.widgets.num_flatfields, set_value=self.scan.num_flatfields
    endif
    if(self.scan.num_flatfields eq 0 AND self.scan.fast_scan ne 2) then begin
        self.scan.num_flatfields = 1
        widget_control, self.widgets.num_flatfields, set_value=self.scan.num_flatfields
    endif
    ...
    if (self.scan.fast_scan eq 2) then begin
        ...
        t = caput(self.epics_pvs.otf_trigger+':Prescale', self.scan.rotation_step  * ABS(motor_resolution))
    ...
    ; In case of slow/fast scan
    endif else begin
        ; Set motor speed
        if (self.epics_pvs_valid) then motor_speed = self.scan.rotation_motor->GET_SLEW_SPEED()
        self.scan.motor_speed = motor_speed
        widget_control, self.widgets.motor_speed, set_value=self.scan.motor_speed
        ...
        self.scan.num_points = self.scan.num_angles + $
                                self.scan.num_flatfields*(1+ceil(1.*self.scan.num_angles/self.scan.flatfield_increment))
        ...
        for pass=0, self.scan.num_passes-1 do begin
            for i=0, (self.scan.num_angles / self.scan.num_passes) -1 do begin
                ...
                if ((i mod self.scan.flatfield_increment) eq 0) then begin
                    for j = 0, self.scan.num_flatfields-1 do begin
        ...
        ; if the motor reset is canceled, make sure fast scan does not return to initial position
        if(self.scan.cancel_motor_reset) then begin
            for i = self.scan.num_points - self.scan.num_flatfields - 1, self.scan.num_points - 1 do begin
                   (*self.scan.rotation_array)[i] = self.scan.rotation_stop
            endfor
        endif

        self.scan.num_points = step

        ; determine the fast scan subdivisions
        if (self.scan.fast_scan eq 1) then begin
            arraysize = self.scan.ccd->getProperty('ArraySize_RBV', string = 0)
            self.scan.sub_size = floor(.800*10.^9/(1.0*arraysize))
            ptr_free, self.scan.sub_stop_array
            sub_array_size = ceil(1.0*self.scan.num_points/self.scan.sub_size)+1
            self.scan.sub_stop_array = ptr_new(fltarr(sub_array_size))
    ...
    endelse
    if (self.scan.fast_scan eq 1) then begin
        self.scan.ccd->setProperty,'NumImages',(*self.scan.sub_stop_array)[self.scan.sub_index]
        if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
            t = caput(self.scan.camera_name+':netCDF1:NumCapture',(*self.scan.sub_stop_array)[self.scan.sub_index])
            self.scan.ccd->setProperty,'TriggerMode',0 ; free run internal trigger
            t = caput(self.scan.camera_name+':netCDF1:FileWriteMode',2)
        endif else begin
            self.scan.ccd->setProperty,'TriggerMode',1; external trigger
        endelse
    endif else if (self.scan.fast_scan eq 0) then begin
        self.scan.ccd->setProperty,'NumImages',1
        self.scan.ccd->setProperty,'TriggerMode',0 ; free run internal trigger
        if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
            t = caput(self.scan.camera_name+':netCDF1:NumCapture',1)
            t = caput(self.scan.camera_name+':netCDF1:FileWriteMode',0)
...
pro tomo_collect_ad::scanPoll
    ...
    if (self.scan.current_state eq self.scan.states.ABORT_SCAN) then begin
        self->stop_scan
        ; Fast Scan and Slow Scan State Machine
        ...
        if (self.scan.fast_scan eq 1) then begin
            ...
            ; Slow scan single image capture
            ; write slow scan comments
            ...
            endelse
            if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
                ; For a Slow Scan, record the angle and image type in the data file
                t = caput(self.scan.camera_name+':TIFF1:FileTemplate',[byte(comment1),0B])
                t = caput(self.scan.camera_name+':TIFF1:FilePath',[byte(comment2),0B])

                ; Slow Scan image acquisition
                self.scan.ccd->setProperty, 'Acquire',1
                wait, .01
            endif else begin
                ; For a Slow Scan, record the angle and image type in the data file
        ...
        if (self.scan.fast_scan eq 1) then begin
            ...
            if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
                ...
                ; make sure that detector is on and waiting for trigger
                ccd_busy1 = 0
                ccd_busy2 = 0
                while (~ccd_busy1 OR ~ccd_busy2) do begin
                    wait, .01
        ...
        if (self.scan.fast_scan eq 1) then begin
            ; See if Prosilica detector has finished acquiring
            ccd_busy = 1
            if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
                if (self.scan.ccd->getProperty('DetectorState_RBV', string = 0) eq 0) then begin
                    ccd_busy = 0
                    ; if the acquisition has not yet completed, wait and retry acquisition
                endif else begin
                    acq_time = self.scan.ccd->getProperty('AcquireTime', string = 0)
                    wait, acq_time
                    i = 0
                    ; wait for camera to acknowledge acquisition of current image
                    while (i lt fix(acq_time*5/.01)) do begin
                        wait, .01
                        if (self.scan.ccd->getProperty('Acquire_RBV', string = 0) eq 0) then begin
                            ccd_busy = 0
                            break
                        endif
                        i= i + 1
                    endwhile
                    ; if camera has not captured the current image, acquire again and return to detector_wait_stop
                    if (i ge fix(acq_time*5/.01) AND $
                            self.scan.ccd->getProperty('DetectorState_RBV', string = 0) ne 0) then begin
                        self.scan.ccd->setProperty,'Acquire',1
                        wait, .01
                        return
                   endif
               endelse
                ...
                ; See if Roper detector has started its readout, meaning exposure is done, OK to move motor
                newMonitor = caCheckMonitor(self.epics_pvs.ccd_ready)
                print, 'Waiting for camera readout', self.scan.current_point, newMonitor
            ...
            ccd_busy2 = self.scan.ccd->getProperty('Acquire_RBV',string = 0)
            if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
                ; check if image was actually captured, else return to Motor_Wait to retry capturing
                if (ccd_busy2 ne 0) then begin
                    acq_time = self.scan.ccd->getProperty('AcquireTime', string = 0)
                    i = 0
                    while (i lt fix(acq_time*10/.01)) do begin
                        wait, .01
                        if (self.scan.ccd->getProperty('Acquire_RBV', string = 0) eq 0) then break
                        i = i + 1
                    endwhile
                    if (i ge fix(acq_time*10/.01)) then begin
                        self->set_state, self.scan.states.MOTOR_WAIT
                        return
                    endif
                endif
            endif
            ccd_busy = ccd_busy1 OR ccd_busy2
        endelse
        if (ccd_busy ne 0) then return
        ...
        if (self->check_beam() eq 0) then begin
            ; incremented the file number, we need to decrement it.
        ...
        if (self.scan.fast_scan eq 0) then begin
            if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
                t = caget(self.scan.camera_name+':netCDF1:WriteFile_RBV', busy)
                while (busy) do begin
                    t = caget(self.scan.camera_name+':netCDF1:WriteFile_RBV', busy)
                       wait, .01
                endwhile
        ...
        ; advance the sample stage to the next position
        self.scan.current_point++
        ...
        if (self.scan.fast_scan eq 1) then begin
            if (self.scan.current_point eq (*self.scan.sub_stop_array)[self.scan.sub_index]) then begin
                wait, 1.0
                self.scan.sub_index++
                if (self.scan.sub_index ge n_elements((*self.scan.sub_stop_array))) then return
                if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
                    ; stop acquisition
                    ccd_busy1 = self.scan.ccd->getProperty('DetectorState_RBV', string = 0)
                    ccd_busy2 = self.scan.ccd->getProperty('Acquire_RBV',string = 0)
                    ...
                    ; wait for capture stack to catch up
                    t = caget(self.scan.camera_name+':netCDF1:NumCaptured_RBV',nncdf)
                    t = caget(self.scan.camera_name+':netCDF1:NumCapture',total_ncdf)
                    while (nncdf ne total_ncdf) do begin
                        wait, 1
                        t = caget(self.scan.camera_name+':netCDF1:NumCaptured_RBV',nncdf)
                    ...
                    ; Stop acquisition
                    widget_control, self.widgets.status, set_value='Stopping scan'
                    ;self.scan.ccd->setProperty,'Acquire',0
                    wait, .1
                    widget_control, self.widgets.status, set_value='Waiting for WinView on last frame'

                    ; wait for detector to return to idle state
                    ccd_busy1 = 1
                    ccd_busy2 = 1
                    while (ccd_busy1 OR ccd_busy2) do begin
                        wait, .01
                    ...
                    ; write file and wait for file writeout to finish
                    t = caput(self.scan.camera_name+':cam1:WriteFile',1)
                    wait, .1
                    busy = 1
                    while (busy) do begin
                    ...
                    ; wait for winview to be ready to begin capturing
    ...
    ; OTF scanning state machine
        ...
        self.scan.current_point++
        ; check position of motor, if in final position send to STOP
        motor_position = self.scan.rotation_motor->get_position()
        if (ABS(motor_position - (*self.scan.otf_rotation_array)[n_elements((*self.scan.otf_rotation_array))-1]) $
                      lt 10.^(-5)) then begin
            ; check for when flat field measurements are finished
            ccd_busy1 = 1
            ccd_busy2 = 1
        ...
        ; count initial bad frames
        if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
            t = caget(self.scan.camera_name+':cam1:PSBadFrameCounter_RBV',bad_frames)
            self.scan.bad_frames = bad_frames
            t = caget(self.scan.camera_name+':netCDF1:DroppedArrays_RBV',bad_frames)
            self.scan.bad_frames = self.scan.bad_frames + bad_frames
        endif
        ...
        if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then self.scan.ccd->setProperty, 'TriggerMode', 'Sync In 2' $
        else self.scan.ccd->setProperty, 'TriggerMode',1
...
pro tomo_collect_ad::stop_scan
    ...
    ; In case of Fast scan
    endif else if (self.scan.fast_scan eq 1) then begin
        if(~self.scan.cancel_motor_reset) then self.scan.rotation_motor->move, (*self.scan.rotation_array)[0]
        ...
        if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
            ...
            ; wait for camera to return to an idle state, wait for capturing to stop
            ccd_busy1 = self.scan.ccd->getProperty('DetectorState_RBV', string = 0)
            ccd_busy2 = self.scan.ccd->getProperty('Acquire_RBV',string = 0)
            while (ccd_busy1 OR ccd_busy2) do begin
                wait, .01
    ...
    ; in case of slow scan
        ...
        if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
            ...
            t = caput(self.scan.camera_name+':netCDF1:Capture',0)
            t = caput(self.scan.camera_name+':netCDF1:EnableCallbacks',0)
            t = caput(self.scan.camera_name+':netCDF1:AutoIncrement',0)
            self.scan.ccd->setProperty,'TriggerMode',0
            t = caput(self.scan.camera_name+':netCDF1:FileNumber',1)
            t = caput(self.scan.camera_name+':netCDF1:AutoSave',0)
        endif else begin
    ...
    if (self.scan.cancel_motor_reset) then self.scan.rotation_motor->set_position,self.scan.rotation_start
    ...
    widget_control, self.widgets.status, set_value='Zeroing motors'
    ; in case of Roper camera, wait for winview to return to idle state
    if (self.scan.camera_manufacturer eq self.camera_types.ROPER) then begin
...
pro tomo_collect_ad::move_sample_in
...
pro tomo_collect_ad::move_sample_out
    if (not self.epics_pvs_valid) then return
    if (self.scan.flatfield_axis ne 1) then begin
...

function tomo_collect_ad::validate_epics_pvs
...

function tomo_collect_ad::restore_settings, file
    ncomments = 0
...

pro tomo_collect_ad::update_screen, force_update=force_update
...
pro tomo_collect_ad::epics_pvs_event, event
...

pro tomo_collect_ad::copy_expinfo_to_widgets
...
pro tomo_collect_ad::copy_expinfo_from_widgets
...

pro tomo_collect_ad::copy_settings_to_widgets
    ; These are in self.epics_pvs
    ...
    widget_control, self.widgets.fast_scan,             set_droplist_select=self.scan.fast_scan
    widget_control, self.widgets.motor_speed,            set_value=self.scan.motor_speed
    widget_control, self.widgets.num_flatfields,        set_value=self.scan.num_flatfields
    widget_control, self.widgets.camera_name,            set_value = strtrim(self.scan.camera_name,2)
end


pro tomo_collect_ad::copy_settings_from_widgets
    ; These are in self.epics_pvs
    ...
    value = widget_info(self.widgets.fast_scan, /droplist_select)
...
pro tomo_collect_ad::expinfo_event, event
    if (tag_names(event, /structure_name) eq 'WIDGET_KILL_REQUEST') then begin
               ; Convert event.id to cancel
...
pro tomo_collect_ad::event, event
    if (tag_names(event, /structure_name) eq 'WIDGET_KILL_REQUEST') then begin
    ...
    case event.id of
        ...
        self.widgets.PS_attributes: begin
            file = dialog_pickfile(/write, get_path=path)
            if (file eq '') then return
            widget_control, self.widgets.PS_attributes_filename, set_value = file
            self.scan.PS_attributes_filename = file
        ...
        self.widgets.fast_scan: begin
            ; set value of self.scan.fast_scan index
            self.scan.fast_scan = event.index
            ; set sensitivity of certain widgets depending on fast_scan mode
            if(event.index eq 2) then begin
                widget_control, self.widgets.num_passes, sensitive = 0
            ...
            if(event.index ne 2) then begin
                widget_control, self.widgets.num_passes, sensitive = 1
        ...
        self.widgets.cancel_motor_reset: begin
            self.scan.cancel_motor_reset = event.select
        end
        ...
        self.widgets.camera_name: begin
            widget_control, self.widgets.camera_name, get_value = camera_name
            ...
            if(name eq 'Roper Scientific') then begin
                ...
                self.scan.camera_manufacturer = self.camera_types.PROSILICA
            endif else begin widget_control, self.widgets.status, set_value='Unknown Camera Type'
                return
            endelse

            widget_control, self.widgets.PS_attributes_filename, sensitive = ~self.scan.camera_manufacturer
            widget_control, self.widgets.PS_attributes, sensitive = ~self.scan.camera_manufacturer
            if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
                t = caget(self.scan.camera_name+':cam1:NDAttributesFile', file)
                widget_control, self.widgets.PS_attributes_filename, set_value = strtrim(file,2)
                self.scan.PS_attributes_filename = strtrim(file,2)
            endif else begin
                widget_control, self.widgets.PS_attributes_filename, set_value = ''
            endelse
...
pro tomo_collect_ad::dc_widgets_event, event
    if (tag_names(event, /structure_name) eq 'WIDGET_KILL_REQUEST') then begin
...
function tomo_collect_ad::init
;+
; NAME:
;       tomo_collect_ad::init
;
; PURPOSE:
...
;-
    ...
    ; default camera variables names
    self.scan.camera_name = '13BMDRP1'
    ...
    if (error ne 0) then begin
        ...
        t = caget(self.scan.camera_name + ':cam1:Manufacturer_RBV',name)
        if(name eq 'Prosilica') then begin
            t = caput(self.scan.camera_name+':netCDF1:EnableCallbacks',0)
            t = caget(self.scan.camera_name+':netCDF1:Capture_RBV',capturing)
            if (capturing ne 0) then t = caput(self.scan.camera_name+':netCDF1:Capture',0)
            t = caput(self.scan.camera_name+':netCDF1:NumCapture',1)
    ...
    self.scan.PS_attributes_filename = ''
    ...
    self.widgets.base_filename = widget_button(file, $
                                            value = 'Base filename ...')
    self.widgets.PS_attributes = widget_button(file, $
                                            value = 'Prosilica attributes file ...')
    widget_control, self.widgets.PS_attributes, sensitive = ~self.scan.camera_manufacturer
    ...
    self.widgets.num_groups = cw_field(row, /column, title = 'OTF Groups', $
        ...
        if (self.scan.num_flatfields eq 0) then widget_control, self.widgets.num_groups, set_value = $
                        strtrim(CEIL(1.0*self.scan.num_angles/self.scan.flatfield_increment),2)
        widget_control, self.widgets.num_groups, sensitive=0
    self.widgets.cancel_motor_reset = cw_bgroup(row, /column, ['Cancel Motor Reset'], /nonexclusive, $
                                   set_value = [self.scan.cancel_motor_reset])
    row = widget_base(col0, /row)
    ...
    self.widgets.fast_scan = widget_droplist(col1, $
                                                value=['Slow Scan', 'Fast Scan', 'OTF Scan'])
    widget_control, self.widgets.fast_scan, set_droplist_select=self.scan.fast_scan
    ...
    self.widgets.camera_name = cw_field(row, title="Camera name:", $
                                      xsize=50, value = self.scan.camera_name, /return_events)
    row = widget_base(col0, /row)
    self.widgets.base_file = cw_field(row, title="Output file name:", $
                                      xsize=50, value=self.scan.base_filename, /noedit)
    row = widget_base(col0, /row)
    self.widgets.PS_attributes_filename = cw_field(row, title="Prosilica image attributes .xml file:", $
                                      xsize=50, value=self.scan.PS_attributes_filename, /noedit)
    widget_control, self.widgets.PS_attributes_filename, sensitive = ~self.scan.camera_manufacturer
    if (self.scan.camera_manufacturer eq self.camera_types.PROSILICA) then begin
        t = caget(self.scan.camera_name+':cam1:NDAttributesFile', file)
        widget_control, self.widgets.PS_attributes_filename, set_value = strtrim(file,2)
    ...
    if (self.scan.num_flatfields eq 0) then widget_control, self.widgets.num_groups, set_value = $
            strtrim(CEIL(1.0*self.scan.num_angles/self.scan.flatfield_increment),2)
    widget_control, self.widgets.num_groups, sensitive=(self.scan.fast_scan eq 2)
    widget_control, self.widgets.num_passes, sensitive = (self.scan.fast_scan ne 2)
    t = self->validate_epics_pvs()

    return, 1
end

pro tomo_collect_ad::cleanup

end

pro tomo_collect_ad__define
    ...
    tomo_collect_ad = {tomo_collect_ad, $
        widgets: widgets, $
