...
pro vol_trim_open_volume, Event

    in_file = dialog_pickfile(filter='*.volume', /must_exist, /read, get_path=path)
    if in_file eq '' then return
    cd, path
    out_file = strmid(in_file, 0, strpos(in_file, '.', /reverse_search)) + '-trim.volume'
	...
	state = {text:text, in_file:in_file, out_file:out_file, vol:vol, sx:sx, sy:sy, sz:sz}
    WIDGET_CONTROL, event.TOP, SET_UVALUE=state, /NO_COPY

end


pro vol_trim_find_xy, Event
...
pro vol_trim_trim, Event
    ...
    vol = vol[x_lo:x_up, y_lo:y_up, z_lo:z_up]
	...
	if temp ne '' then begin
		...
		success = write_log(strmid(out_file, 0, strlen(out_file)-6)+'log', [x_lo, x_up, y_lo, y_up, z_lo, z_up])
...
pro vol_trim_base, GROUP_LEADER=wGroup, _EXTRA=_VWBExtra_

  vol_trim_base = Widget_Base( GROUP_LEADER=wGroup, UNAME='vol_trim_base', TITLE='vol_trim'  $
      ,/column , /align_left, TAB_MODE=1, SPACE=7 ,XPAD=5 ,YPAD=5, xoffset=100, yoffset=100)


  Open_Button = Widget_Button(vol_trim_base, UNAME='Open_Button' ,/ALIGN_left ,VALUE=' Open volume:', uvalue='open_button', ysize=25)

  File_Label = Widget_Text(vol_trim_base, UNAME='File_Label' ,FRAME=0 ,scr_xsize=310 ,/ALIGN_LEFT ,VALUE=' Click button to start.')

  Cols_base = widget_base(vol_trim_base, /row, space=10, /align_left, frame=0, xpad=0, ypad=0)
  ...
  Trim_Button_Base = widget_base(vol_trim_base, /row, /align_center, space=20)
  Trim_Button = Widget_Button(Trim_Button_Base, UNAME='Trim_Button' ,SCR_XSIZE=120 ,SCR_YSIZE=25  $
      ,/ALIGN_CENTER ,VALUE='Trim volume!', uvalue='trim', sensitive=0)

  Widget_Control, /REALIZE, vol_trim_base

  state = {initialized:0}
  WIDGET_CONTROL, vol_trim_base, SET_UVALUE=state

  XManager, 'vol_trim_base', vol_trim_base, /NO_BLOCK

end
;
; Empty stub procedure used for autoloading.
;
pro vol_trim, GROUP_LEADER=wGroup, _EXTRA=_VWBExtra_
  vol_trim_base, GROUP_LEADER=wGroup, _EXTRA=_VWBExtra_
end
